...
| 2026-08-30 | Phase 5 of the persons plan opened two manual boxes: resize the window slowly across a derivative-switch threshold and confirm the boxes track the photo; and check on a HiDPI display, where `rvas_choose()` removes `usemap` and takes a different branch. | **First box replaced 2026-08-30** by `plugins/persons/tests/e2e/overlay.spec.js` — `box geometry matches the region at 1400px` / `at 800px` (fresh load at each width) and `the boxes follow the photo when a smaller derivative is loaded`, which drives the theme's own `changeImgSrc()` rather than assigning a `src`. All three were watched red against a 10px offset added to the overlay's `left`, with the container confirmed to have picked up the bytes (`md5sum` polled against the host) and `_data/combined` cleared first. A viewport resize is deliberately *not* what that spec asserts on: whether it changes the rendered size at all depends on how large the fixture photo happens to be, so it could not carry an anti-vacuity check. The HiDPI box stays open — see below. |
...
| The person overlay on a **HiDPI** display, where `rvas_choose()` takes its `devicePixelRatio > 1` branch and removes `usemap` | Playwright can set `deviceScaleFactor`, so the branch itself is reachable — what is not is the thing the check is for: whether the boxes look right on a real high-density panel. The falsifiable half is already covered without it, because the overlay is placed from `getBoundingClientRect()`, which is in CSS pixels either way, and the branch changes only which derivative URL and which `usemap` attribute the photo carries. The one behaviour that branch *does* change and this suite would notice — that a click outside a box still navigates — is asserted at 1x, where the `<area>` map is present; at 2x the theme's own JavaScript handler takes over, which is core's code on a page the plugin does not modify. |
