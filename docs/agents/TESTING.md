...
| 2026-08-31 | Phase 7 of the FTP deploy plan left one manual box: *a fresh reader can deploy from `tools/deploy/README.md` alone*. | **Half automated the same day** as `tools/deploy/tests/test_readme.py` (16 tests), which checks every claim in the README that is a second copy of something production states: each documented flag is accepted by `build_parser()` **and** each accepted flag is documented, each exit code maps to an `errors.py` class, each credential field matches `deploy.example.json` in both directions, the three defaults match `config.py`, every exclusion prefix named is really in `fileset.EXCLUDED_PREFIXES`, the two published exceptions really survive `select()`, and every relative link resolves. Each of the 16 was watched red against a mutation of the thing it watches. **Two survived their first mutant** and were strengthened rather than recorded as killed: both asserted only that a string appears *somewhere*, so a stale second mention of the same file passed — they now read the sentence that makes the promise and compare a whole set. The parser itself was caught by its own lower-bound guards on the first run: scanning the raw Markdown pairs backticks off by one after the first ``` fence, so the flag scan found **zero** flags and said so instead of passing vacuously. What stays open is the half no assertion reaches — whether the prose is *followable* by a person who has not read the code. |
