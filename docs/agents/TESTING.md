...
`handbuch/` documents five workflows. Reconciling every control it names against the
...
| 2026-08-31 | The FTPS adapter (`FtplibTransport`) against the real web space: `AUTH TLS` negotiated, `PROT P` set, one small file put, listed, downloaded byte-identical, deleted. | Not automatable here — there is no FTP server in this repository and no CI to run one in. The adapter is kept decision-free precisely so the hand check is all that is missing: everything it would otherwise decide lives in `config.py`, `fileset.py`, `manifest.py` and `urls.py` and is unit-tested, and `FakeTransport` exercises `upload.run()`'s whole state machine. What the run cannot prove is that `ftplib` speaks to *that* server, which is the one thing a double never proves about an adapter. |
| 2026-08-31 | The first real deploy end to end: 3332 files / 128.4 MB uploaded in 823.3 s, `install installed`, `typetags/provenance/persons activated`, `sync 105 photos, 4 albums, 0 errors`, then the gallery opened in a browser — four album tiles summing to 105, derivatives generated from `_data/i/`, album and picture pages with zero console errors. Screenshots in `.agent-tests/2026-08-31-ftp-deploy-verification/`. A second run then reported `0 new, 0 changed, 3332 unchanged`, `already installed - skipped`, `sync 0 photos`, in 2.6 s. | Not automatable — it needs a real web space, a real database and a real FTPS server, none of which this repository can provision. **It earned its keep twice over**: the second run exposed two defects no fake could reach (the generated `config.inc.php` classified `removed` on every run after the first, and `.ddev/` plus the fork's hook scripts being published), and a Ctrl-C experiment exposed a third (a raw `KeyboardInterrupt` traceback out of `ssl.unwrap`). All three are now regression-tested — `test_the_generated_config_is_absent_from_every_diff_bucket`, `test_excludes_the_whole_tools_directory`, `test_an_interrupted_run_says_it_can_be_resumed` — so what stays unautomated is only the transport and the remote, not the rules they exposed. |
| 2026-08-31 | The two host capability probes ([`.claude/rules/deployment.md`](../../.claude/rules/deployment.md) names the method): a `phpinfo()` file and a capability file uploaded under random 16-byte names through the tool's own `FtplibTransport`, fetched over HTTP, and deleted in a `finally` that re-checked the URL no longer answers; then `admin.php?page=maintenance&action=phpinfo` from inside the install. Measured on `bilder.foerderverein-sefferweich.de`: `disable_functions` **empty**, exiftool **12.76** at `/usr/bin/exiftool`, Imagick **3.7.0** (ImageMagick 6.9.12-98 Q16), GD with JPEG/PNG/WebP, PHP 8.4.16, `memory_limit` 384M, no `open_basedir`. | Deliberately not automated, and this is the entry to read before anyone proposes it: automating it means leaving a PHP file that reports the host's configuration at a URL on a public web space. A five-minute hand check with a random name and a `finally` that deletes is the safer trade. The **finding** is that both open questions resolved in favour of the host — `exec()` is available and Imagick is present, so the provenance and persons write-back works there, where the plan had been written assuming the degraded mode might be permanent. The gap left behind: the host has exiftool 12.76 against the local 13.25, and no suite has ever run against 12.76. |
| 2026-08-31 | Phase 7 of the FTP deploy plan left one manual box: *a fresh reader can deploy from `tools/deploy/README.md` alone*. | **Half automated the same day** as `tools/deploy/tests/test_readme.py` (16 tests, extended to **19** on 2026-08-31 — see the row below), which checks every claim in the README that is a second copy of something production states: each documented flag is accepted by `build_parser()` **and** each accepted flag is documented, each exit code maps to an `errors.py` class, each credential field matches `deploy.example.json` in both directions, the three defaults match `config.py`, every exclusion prefix named is really in `fileset.EXCLUDED_PREFIXES`, the two published exceptions really survive `select()`, and every relative link resolves. Each of the 16 was watched red against a mutation of the thing it watches. **Two survived their first mutant** and were strengthened rather than recorded as killed: both asserted only that a string appears *somewhere*, so a stale second mention of the same file passed — they now read the sentence that makes the promise and compare a whole set. The parser itself was caught by its own lower-bound guards on the first run: scanning the raw Markdown pairs backticks off by one after the first ``` fence, so the flag scan found **zero** flags and said so instead of passing vacuously. What stays open is the half no assertion reaches — whether the prose is *followable* by a person who has not read the code. |
| 2026-08-31 | Reconciling the FTP deploy plan against the suite it describes: every test name the plan cites, checked against the names the suite really defines. | **Run once by hand, and deliberately not automated.** Two citations pointed at tests that do not exist — `test_generated_config_is_valid_php_open_tag`, whose assertion landed inside `test_generated_config_carries_the_three_settings`, and `test_excludes_the_fork_s_own_hook_scripts_but_keeps_upstream_tools`, superseded by `test_excludes_the_whole_tools_directory` when decision 0022 landed. Both fixed in place. A standing test for this would be *a scan that reads a document looking for a word*, which `test-design.md` forbids, and a plan is a dated snapshot that is supposed to age rather than be frozen green. **The gap it exposed was real and is now guarded**: `tools/deploy/README.md` and `.claude/rules/deployment.md` both quote the suite size as a dated measurement and neither was checked — the number had already rotted once (288 after the suite reached 304, corrected by hand in Phase 7). `test_the_documented_test_count_is_the_number_pytest_reports` (parametrised over both documents, compared against `pytest --collect-only` in its own process, because counting `def test_` would miscount the parametrised cases) and `test_both_documents_date_the_same_measurement` now do. Red on arrival for the right reason — the three new tests moved the count 304 → 307 — then each watched red against its own mutant: a differing date, and the count sentence deleted from the rules file, which failed as "states no dated test count" rather than passing on an empty scan. |
| 2026-09-01 | Phase 3 of the deploy state-guards plan left two manual boxes, both against `bilder.foerderverein-sefferweich.de`: a real run with the manifest in place passes the new preflight, and a run with the manifest missing refuses. The agree path printed `preflight   installed — manifest and remote agree` and proceeded — 27 new / 1 changed sent, install skipped, all three plugins active, and **no `galleries` line**, so nothing irreplaceable was pruned. The refusal path was then forced by moving `.state/<host>_root.json` aside: `EXIT=9`, the message named the 3335 files the run would have re-uploaded and both `--adopt-remote-state` and `--audit`, and the report **stopped after the `manifest … (new)` line** — no `transport` line, so no FTPS connection was opened. Manifest restored. | Not automatable — the same limit as every other row here: no FTP server, no second web space, and no way to give a double the property being checked, which is that *that* remote's `install.php` answers the marker at all. The falsifiable half already is: all four cells of `check_state`'s decision table have their own test, `test_a_wiped_remote_exits_with_the_state_mismatch_code` reads the code off `StateMismatchError`, `test_a_refused_preflight_uploads_nothing` asserts `transport.calls == []`, and `test_dry_run_connects_to_nothing` proves the new step did not cost the dry run its offline property. What the hand run adds over those is the ordering an operator sees — that the abort happens before the transport line, not after a 128 MB upload. |
...
| Whether each of the five documented workflows can actually be completed by following only `handbuch/`, without reading the code | The oracle is a first-time reader, not a machine. What a machine can decide is checked by `handbuch/tools/check.php`: every reference resolves, every screenshot is referenced, every page is well formed, and every `admin.php?page=` route quoted in the text resolves the way `admin.php` resolves it. Whether a step is *missing* from a sequence is not among those. |
