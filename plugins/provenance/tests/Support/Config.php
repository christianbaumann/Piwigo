...
class Config
{
    ...
    {
        return getenv('PROVENANCE_TEST_BASE_URL') ?: 'http://localhost';
    }

    public static function dbHost(): string
    {
        return getenv('PROVENANCE_TEST_DB_HOST') ?: 'db';
    }

    public static function dbUser(): string
    {
        return getenv('PROVENANCE_TEST_DB_USER') ?: 'db';
    }

    public static function dbPassword(): string
    {
        return getenv('PROVENANCE_TEST_DB_PASSWORD') ?: 'db';
    }

    public static function dbName(): string
    {
        return getenv('PROVENANCE_TEST_DB_NAME') ?: 'db';
    }

    /** The webmaster account the admin-gated suites log in as. */
    public static function username(): string
    {
        return self::required(TestUsers::envVars('provenance_webmaster')[0]);
    }

    public static function password(): string
    {
        return self::required(TestUsers::envVars('provenance_webmaster')[1]);
    }

    /** An authenticated non-admin, for the [NEG] side of every admin gate. */
    public static function normalUsername(): string
    {
        return self::required(TestUsers::envVars('provenance_normal')[0]);
    }

    public static function normalPassword(): string
    {
        return self::required(TestUsers::envVars('provenance_normal')[1]);
    }
    ...
    private static function required(string $envVar): string
    {
        ...
        if ($value === false || $value === '')
        {
            throw new RuntimeException(
                ...
                'Run `ddev exec php plugins/provenance/tests/Support/create-test-users.php` to create ' .
